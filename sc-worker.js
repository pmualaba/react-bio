...
const logic = require('./dna/router').logic
...
class Worker extends SCWorker {
    run() {
        ...
        db.json = new JsonDB('dna/package.core.db/db', true, true, '/')
        ...
        scServer.addMiddleware(scServer.MIDDLEWARE_HANDSHAKE_SC, function(req, next) {
            ...
            const jwt = req.socket.request.headers.cookie && req.socket.request.headers.cookie.split('_jwt=')[1].split(';')[0]
        ...
        scServer.on('connection', function(socket, status) {
            ...
            socket.on('/api', async function(req, res, next) {
                ...
                logic[req.body.meta.package][req.body.meta.endpoint][req.body.type](req, res, next, meta)
            })
